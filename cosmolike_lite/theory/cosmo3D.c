...
#define wa_max_emu 1.35
#define wa_min_emu 1.55 // value correspond to -(w_0+w_a)^(1/4)
...
double growfac(double a)
{
  ...
  if (recompute_expansion(C))
  {
    ...
    if (table != 0)
      free_double_vector(table, 0, Ntable.N_a - 1);
    if (ai != 0)
      free_double_vector(ai, 0, Ntable.N_a - 1);
...
double Delta_L_wiggle(double k)
{
  ...
  else
  {
    if (recompute_Delta(C))
    {
      ...
      if (table_P != 0)
        free_double_vector(table_P, 0, Ntable.N_k_lin - 1);
...
double p_lin(double k, double a)
{
  ...
  if (recompute_cosmo3D(C))
  {
    update_cosmopara(&C);
    if (table_P_Lz != 0)
      free_double_matrix(table_P_Lz, 0, Ntable.N_a - 1, 0, Ntable.N_k_lin - 1);
  ...
  if (k / cosmology.coverH0 > exp(logkmax))
    return 0.0;
...
void nonlin_scale(double amp, double *R_NL, double *neff, double *Curv)
{
  ...
  while (converged == 0)
  {
    ...
    if (sig_R > 1.0)
      logRmin = log10(array[0]);
    if (sig_R < 1.0)
      logRmax = log10(array[0]);
    iterstep = iterstep + 1;
    if (fabs(sig_R - 1.0) < 0.0001 || iterstep > itermax)
      converged = 1;
  }
...
double Pdelta(double k_NL, double a)
{
  ...
  if (P_type == -1)
  {
    if (strcmp(pdeltaparams.runmode, "Halofit") == 0)
      P_type = 0;
    if (strcmp(pdeltaparams.runmode, "halofit") == 0)
      P_type = 0;
  }
...
double chi(double a)
{
  ...
  if (res < 0)
  {
    printf("interpolation error in chi(%e)\n", a);
    res = 0.01;
  }
  return res;
}
...
double a_chi(double chi1)
{
  ...
  if (chi1 > chi_max)
  {
    printf("called a_chi(chi) with chi > chi(limits.a_min\nEXIT\n");
    exit(1);
  }
...
double f_K(double chi)
{
  ...
  if (K > precision.medium)
  {                /* open */
    K_h = sqrt(K); // K in units H0/c see BS eq. 2.30
    f = 1. / K_h * sin(K_h * chi);
    // printf("open\n");
  }
  else if (K < -precision.medium)
  { /* closed */
    K_h = sqrt(-K);
