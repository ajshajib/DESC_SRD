...
def init_Y1_shear():
    file_source_z = os.path.join(
        dirname, "zdistris/zdistri_model_z0=1.300000e-01_beta=7.800000e-01_Y1_source"
    )
    file_lens_z = os.path.join(
        dirname, "zdistris/zdistri_model_z0=2.600000e-01_beta=9.400000e-01_Y1_lens"
    )
...
def init_Y10_shear():
    file_source_z = os.path.join(
        dirname, "zdistris/zdistri_model_z0=1.100000e-01_beta=6.800000e-01_Y10_source"
    )
    file_lens_z = os.path.join(
        dirname, "zdistris/zdistri_model_z0=2.800000e-01_beta=9.000000e-01_Y10_lens"
    )
...
def get_fisher_matrix(FM_params, invcov, flag, step_width=1.0):
    ...
    for n, p in enumerate(FM_params):
        ...
        if p in InputCosmologyParams().names():
            ...
            print(
                "FM: evaluting derivative for parameter %s=%e with prior %e (%e)"
                % (p, p0, diag_prior_Fisher[n], 1.0 / np.sqrt(diag_prior_Fisher[n]))
            )
            # print(p0, dp)
            ...
            diag_prior_Fisher[n] = 1.0 / (getattr(nuisance_prior, pshort)[i]) ** 2.0

            print(
                "FM: evaluting derivative for parameter %s[%d]=%e with prior %e (%e)"
                % (p, i, p0, diag_prior_Fisher[n], 1.0 / np.sqrt(diag_prior_Fisher[n]))
            )
            ...
            diag_prior_Fisher[n] = 1.0 / (getattr(nuisance_prior, p)) ** 2.0
            print(
                "FM: evaluting derivative for parameter %s=%e with prior %e (%e)"
                % (p, p0, diag_prior_Fisher[n], 1.0 / np.sqrt(diag_prior_Fisher[n]))
            )
        ...
        derivs[n, :] = (-dv_pp + 8.0 * dv_p - 8.0 * dv_m + dv_mm) / (12.0 * dp)
...
def FM_analyze(FM, FM_params):
    ...
    if (ind_w0 > -1) & (ind_wa > -1):
        FOM = 1.0 / np.sqrt(
            FMinv[ind_w0, ind_w0] * FMinv[ind_wa, ind_wa]
            - FMinv[ind_w0, ind_wa] * FMinv[ind_wa, ind_w0]
        )
...
def FM_parameter_bias(file_datav_biased, file_datav_fid, invcov, FM, derivs, FM_params):
    ...
    if (ind_w0 > -1) & (ind_wa > -1):
        ...
        FMw = LA.inv(
            np.array(
                [
                    [FMinv[ind_w0, ind_w0], FMinv[ind_w0, ind_wa]],
                    [FMinv[ind_wa, ind_w0], FMinv[ind_wa, ind_wa]],
                ]
            )
        )
    ...
    print(
        "Rescaling factor = %e" % (1.0 / (np.sqrt(np.dot(np.dot(deltaw, FMw), deltaw))))
    )
    return 1.0 / (np.sqrt(np.dot(np.dot(deltaw, FMw), deltaw)))
...
if sys.argv[1] == "Y1":
    if sys.argv[2] == "clusterN_clusterWL":
        ...
        MORPRIOR = [
            [1302.77777778, 1319.88833456, 456.13561437],
            [1319.88833456, 1725.21929159, 590.65347498],
            [456.13561437, 590.65347498, 228.8004921],
        ]  # April 8 LSST Y1
    elif sys.argv[2] == "pos_pos":
        invcov = init_Y1_clustering()
    elif sys.argv[2] == "3x2pt":
        invcov = init_Y1()
    elif sys.argv[2] == "shear_shear":
        invcov = init_Y1_shear()
...
if sys.argv[1] == "Y10":
    if sys.argv[2] == "clusterN_clusterWL":
        ...
        MORPRIOR = np.zeros((3, 3))
        MORPRIOR = [
            [2622.22222222, 2839.59646683, 893.79987381],
            [2839.59646683, 3454.43840371, 1097.45127586],
            [893.79987381, 1097.45127586, 424.44951605],
        ]  # April 8 LSST Y10
    elif sys.argv[2] == "pos_pos":
        invcov = init_Y10_clustering()
    elif sys.argv[2] == "3x2pt":
        invcov = init_Y10()
    elif sys.argv[2] == "shear_shear":
        invcov = init_Y10_shear()
...
if sys.argv[2] == "clusterN_clusterWL":
    FM_params = sample_cosmology_clusterN_clusterWL_SRD(MG=False)
if sys.argv[2] == "pos_pos":
    FM_params = sample_cosmology_clustering_SRD(get_N_tomo_clustering(), MG=False)
if sys.argv[2] == "3x2pt":
    FM_params = sample_cosmology_2pt_SRD(get_N_tomo_clustering(), MG=False)
if sys.argv[2] == "shear_shear":
    FM_params = sample_cosmology_shear_SRD(MG=False)
